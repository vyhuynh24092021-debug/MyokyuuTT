...
class Zefoy:
    ...
    def send_captcha(self, new_session = False):
        ...
        captcha_solve = self.solve_captcha('captcha.png')
        ...
        while True:
            ...
            if 'Session expired. Please re-login' in self.video_info:
                ...
                while time.time() - 2 < _ + zefoy:
                    t -= 1
                    print('\033[31m[HaiTool]=>Please wait: \033[1;32m{0}\033[31m '.format(t) + 'second\033[1;32m', end='\r')
                    time.sleep(1)
                continue
    ...
    def get_video_info(self):
        request = self.session.get(f'https://tiktok.livecounts.io/video/stats/{urlparse(self.url).path.rpartition("/")[2]}', 
                                    headers={'authority':'tiktok.livecounts.io', 'origin':'https://livecounts.io', 'user-agent':self.headers['user-agent']}, 
                                    timeout=60).json()
    ...
    def get_video_id(self, url_ = None, set_url=True):
        ...
        request = requests.get(f'https://api.tokcount.com/?type=videoID&username=https://vm.tiktok.com/{url}', 
                               headers={'origin': 'https://tokcount.com', 'authority': 'api.tokcount.com', 'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Safari/537.36'}, 
                               timeout=60)
        if request.text == '':
